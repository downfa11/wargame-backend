...
import com.ns.player.adapter.axon.command.MembershipElo;
import com.ns.player.adapter.axon.command.UpdateEloCommand;
import com.ns.common.GameResultSavedEvent;
import com.ns.common.RollbackGameResultEvent;
import com.ns.player.adapter.axon.event.RollbackUpdateEloEvent;
import com.ns.player.adapter.out.persistence.Player;
import com.ns.player.application.port.out.FindPlayerPort;
import com.ns.player.application.service.EloService;
...
public class GameFinishedSaga {
    ...
    public void handle(GameFinishedEvent event) {
        ...
        CreateResultEvent createResultEvent = convertToCreateResultEvent(event);
        Mono.fromRunnable(() -> eventGateway.publish(createResultEvent)) // Result 서비스의 ResultEventHandler
               .subscribe();
    }

    private CreateResultEvent convertToCreateResultEvent(GameFinishedEvent event) {
        return new CreateResultEvent(
                event.getSpaceId(),
                event.getWinTeam(),
                event.getLoseTeam(),
                event.getBlueTeams(),
                event.getRedTeams(),
                event.getDateTime(),
                event.getGameDuration(),
                event.getChannel(),
                event.getRoom(),
                event.getTimelineData()
        );
    }



    // ElasticSearch에 기록하는데 성공하면 통계 업데이트를 진행
    @SagaEventHandler(associationProperty = "spaceId")
    public void handle(GameResultSavedEvent event) {
        ...
        // 실제로 위의 주석 처리에서 resutl-query로 전달하고 나면, 아래의 로직을 거기서 실행해야함. 아래는 테스트 용도로 현 서비스에서 진행
